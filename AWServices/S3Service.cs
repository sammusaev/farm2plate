...
namespace farm2plate.AWServices
{
    public class S3Service
    {
        ...
        public S3Service()
        {
            ...
            // Make sure that the names of the variables match the ones in your .env
        ...
        public async Task<(bool,string)> CreateBucketAsync()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Whoopsie the status is {resp.HttpStatusCode.ToString()}");
        ...
        public async Task<(bool, string)> UploadToBucket(IFormFile file)
        {
            ...
            PutObjectRequest request = new PutObjectRequest { 
                BucketName = BucketName,
                Key =  key + System.IO.Path.GetExtension(file.FileName),
            ...
            if (resp.HttpStatusCode != System.Net.HttpStatusCode.OK)
            {
                System.Diagnostics.Debug.WriteLine($"Error: {resp.HttpStatusCode.ToString()}");
                return (false, $"{resp.HttpStatusCode.ToString()}");
            }
            return (true, key);
